...
| `signup`                   | /cas/api/v1/user/signup          | User registration           | N            |
...
The app generates a UUID as the userId locally and calls this API. The server stores the userId as both the user identifier and PII. The response body is empty; the app retains the locally generated userId.
...
```c#
def object Signup: "Signup Request"
{
    + string "userId"   : "UUID generated by the app (becomes user identifier and PII)"
    + string "walletId" : "CA app device identifier"
}
```
...
1. Store userId as `userIdentifier` and `pii`.
2. Store walletId associated with the user.

**■ Status 200 - Success**

```c#
def object _Signup: "Signup Response"
{
    // no data
}
```
...
| Code         | Description                          |
| ------------ | ------------------------------------ |
| SCRVCFA00402 | "User identifier already registered." |

**■ Status 500 - Server error**

| Code         | Description                                   |
| ------------ | --------------------------------------------- |
| SCRVCFA00806 | "Failed to process ‘signup’ API request."     |
