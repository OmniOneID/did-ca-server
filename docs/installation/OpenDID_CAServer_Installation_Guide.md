...
- `fabric.configFilePath:`: 
  - Sets the path where Hyperledger Fabric's connection information file is located. This file is automatically generated when installing the Hyperledger Fabric test network, with the default filename being 'connection-org1.json'.
  - Example: {yourpath}/connection-org1.json

- `fabric.privateKeyFilePath:`: 
  - Sets the private key file path used by Hyperledger Fabric clients for transaction signing and authentication on the network. This file is automatically generated when installing the Hyperledger Fabric test network.
  - Example: {yourpath}/{private key filename}

- `fabric.certificateFilePath:`: 
  - Sets the path where the Hyperledger Fabric client certificate is located. This file is automatically generated when installing the Hyperledger Fabric test network, with the default filename being 'cert.pem'.
  - Example: /{yourpath}/cert.pem

- `fabric.mychannel:`: 
  - The name of the private network (channel) used in Hyperledger Fabric. You must set the channel name entered during Hyperledger Fabric test network installation.
  - Example: mychannel

- `fabric.chaincodeName:`: 🔒
  - The name of Open DID's chaincode used in Hyperledger Fabric. This value is fixed as 'opendid'.
  - Example: opendid
